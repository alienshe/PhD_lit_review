...
acbr_plot<-basemap(limits = -55,glaciers = TRUE,shapefiles="Antarctic",land.col = "red")+
  #theme(panel.ontop = FALSE)+
...
pnts <- pnts_sf %>% mutate(
  intersection = as.integer(st_intersects(geometry, ACBR))
  , area = if_else(is.na(intersection), '', ACBR$ACBR_NAME[intersection])
) 
