...
pnts_sf <- st_as_sf(antarctic_subset, coords = c('lon','lat'))
...
num_points_near_ACBR<-data.frame("ACBR"=c(1:15),"ACBR_NAME"=ACBR_combined$ACBR_NAME)
num_points_near_ACBR<-merge(x=num_points_near_ACBR,y=point_count_table,by.x = "ACBR",by.y="row.names",all.x = TRUE,all.y=FALSE)


colnames(num_points_near_ACBR)[2]<-"num_points"
...
ggplot(data=ACBR_combined,aes(ACBR_NAME,num_points))+
  geom_col(aes(fill=ACBR_NAME))+
  scale_fill_manual(values=color_assignments,guide="none")+
  geom_text(aes(label=num_points),vjust=-.6)+
...
```
```{r}
# Source - https://stackoverflow.com/a/78381426
# Posted by L Tyrone, modified by community. See post 'Timeline' for change history
# Retrieved 2026-02-09, License - CC BY-SA 4.0

# Create 'centroids' of municipalities (you can safely ignore the warning)
sf_centroids <- ACBR_combined %>% st_point_on_surface()

pos_cent<-sf_centroids %>% subset(st_coordinates(.)[,1]>0)

# Create points for label locations
label_pos <- sf_centroids %>%
  st_transform("EPSG: 4326") %>%
  mutate(lat = -60,
         lon = st_coordinates(.)[,1]) %>%
  st_drop_geometry()|> st_as_sf(coords = c("lon", "lat")) %>%
  st_set_crs(st_crs("EPSG: 4326"))|>
  st_transform("ESRI:102021")


```
```{r}
sf_lines <- rbind(label_pos[,"ACBR_NAME"], 
                  sf_centroids[,"ACBR_NAME"]) %>%
  group_by(ACBR_NAME) %>%
...
basemap(limits = -55,glaciers = FALSE,shapefiles="Antarctic",land.col = "white",land.border.col = "darkgrey")+
  #theme(panel.ontop = FALSE)+
  annotation_spatial(ACBR_combined, aes(color = ACBR_NAME,fill=ACBR_NAME))+
  annotation_spatial(data = label_pos,color = "black")+ 
  annotation_spatial(data = sf_lines,color = "black")+ 
  scale_colour_manual(values = color_assignments,guide="none")+
  scale_fill_manual(values=color_assignments,guide="none")
