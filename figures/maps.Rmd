...
antarctic_data<-lit_rev_data[lit_rev_data$`Antarctic?...2`=="Yes",]
```
...
```{r}
library(ggguides)
claims_df<-data.frame(id=c("Australia1","Australia1"),country=c("Australian Antarctic Territory","Australian Antarctic Territory"),lat1=c(160,136),lat2=c(142,44.633333))
i=1

Australia_1<-data.frame(lat = c(-89.999, -60, rep(-60,length(142:160)), -60, -89.999), lon = c(142, 142,c(142:160), 160,142),id="Australia1",country="Australia")
Australia_2<-data.frame(lat = c(-89.999, -60, rep(-60,length(45:136)),-60, -89.999), lon = c(44.633333,44.633333,c(45:136),136,44.633333),id="Australia2",country="Australia")
claims<-rbind(Australia_1,Australia_2)


claims_sf <- sfheaders::sf_polygon(
  obj = claims
  , x = "lon"
  , y = "lat"
  , polygon_id = "country"
)
line_assignments<-c('Australian Antarctic Territory'="dashed")
st_crs(claims_sf)<-"EPSG: 4326"
claims_sf<-st_transform(claims_sf,"ESRI:102021")

mawson_station <- data.frame(lat = -67.6033, lon = 62.8742,name="Mawson Station")

mawson_station <- st_as_sf(mawson_station, coords = c('lon','lat'))
st_crs(mawson_station)<-"EPSG: 4326"
mawson_station<-st_transform(mawson_station,"ESRI:102021")
mawson_station_df <- cbind(mawson_station, st_coordinates(st_centroid(mawson_station$geometry)))
stations_map<-basemap(limits = -55,glaciers = FALSE,shapefiles="Antarctic",land.col = "white",land.border.col = "darkgrey")+
  #theme(panel.ontop = FALSE)+
  annotation_spatial(ACBR_combined, aes(color = ACBR_Name,fill=ACBR_Name),size=1.3)+
  scale_colour_manual(values = color_assignments)+
  scale_fill_manual(values=color_assignments)+
  annotation_spatial(mawson_station,color="black")+
  ggrepel::geom_text_repel(data= mawson_station_df,aes(x=X, y=Y, label=name),
    color = "black", check_overlap = FALSE,nudge_x = 3000000,nudge_y = 500000)+
  annotation_spatial(claims_sf,alpha=0, aes(linetype="Australian Antarctic Territory"))+
  scale_linetype_manual(values=line_assignments)+
  labs(fill="Ice-Free ACBR",color="Ice-Free ACBR",linetype=element_blank())+
  legend_left(by = c("fill")) +
  legend_left(by=c("color"))+
  legend_bottom(by = "linetype")+
  theme(legend.text = element_text(size=8))+
  #legend_style(size = 5,by="fill",)+
  #legend_style(size = 5,by="color")+
  theme(legend.text = element_text(size=8))
stations_map
